...
static void (*g_CheckVkResult)(VkResult err) = nullptr;

static VkCommandBuffer g_CommandBuffer = nullptr;
...
void ImGui_ImplGlfwVulkan_RenderDrawLists(ImDrawData* draw_data)
{
	...
	// Upload Vertex and index Data:
	{
		...
		range[0].size = VK_WHOLE_SIZE;// vertex_size;
		range[1].sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
		range[1].memory = g_IndexBufferMemory[g_FrameIndex];
		range[1].size = VK_WHOLE_SIZE;// index_size;
	...
	// Setup scale and translation:
	{
		...
		translate[0] = -1.0f;
		translate[1] = -1.0f;
		vkCmdPushConstants(g_CommandBuffer, g_PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, sizeof(float) * 0,
		                   sizeof(float) * 2, scale);
		vkCmdPushConstants(g_CommandBuffer, g_PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, sizeof(float) * 2,
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
		{
			...
			else
			{
				...
				vkCmdBindDescriptorSets(g_CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, g_PipelineLayout, 0, 1, desc_set, 0,
				                        nullptr);
...
void* ImGui_ImplGlfwVulkan_AddTexture(VkSampler sampler, VkImageView image_view)
{
	VkResult err;
...
bool ImGui_ImplGlfwVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
	...
	VkDescriptorSet font_descriptor_set = (VkDescriptorSet)ImGui_ImplGlfwVulkan_AddTexture(g_FontSampler, g_FontView);
...
bool ImGui_ImplGlfwVulkan_CreateDeviceObjects()
{
	...
	attribute_desc[0].offset = (size_t)(&((ImDrawVert*)nullptr)->pos);
	attribute_desc[1].location = 1;
	attribute_desc[1].binding = binding_desc[0].binding;
	attribute_desc[1].format = VK_FORMAT_R32G32_SFLOAT;
	attribute_desc[1].offset = (size_t)(&((ImDrawVert*)nullptr)->uv);
	attribute_desc[2].location = 2;
	attribute_desc[2].binding = binding_desc[0].binding;
	attribute_desc[2].format = VK_FORMAT_R8G8B8A8_UNORM;
	attribute_desc[2].offset = (size_t)(&((ImDrawVert*)nullptr)->col);
